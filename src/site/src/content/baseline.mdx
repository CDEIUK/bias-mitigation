...
We train simple models on each data set to serve as a baseline before we try any of the possible interventions. You can see the details for the finance use case [here][binder-finance-baseline] and the recruiting use case [here][binder-recruiting-baseline].
