...
The reduction in equalised odds is more significant, reducting equalised odds difference from 0.128 to 0.074.

<PlotLoader source={"finance/interventions/ftu-eo"} />

#### Equal opportunity

The reduction in equal opportunity is identival to equalised odds.
...
Equalised odds could be roughly halved, from 0.133 to 0.064, which is a significant reduction. 
...
The intervention has overall an small influence on loss in accuracy, however also a limited influence to the improvement in fairness. It is therefore useful to consider FTU not as a single fairness intervention technique, but in combination with other techniques.
