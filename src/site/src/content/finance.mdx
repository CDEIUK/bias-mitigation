...
The data we have available to us is the Adult data set from the [UCI Machine learning repository][adult-data]. The task is to predict whether or not an individual is a high earner from their demographics.
...
Finally, we identify a possible "legitimate risk factor" in the `hours_per_week` feature. There is a clear relationship between the number of hours worked per week and the chances that the individual is a high earner, which stands to reason. Of course we should be careful, because the number of hours worked per week by any one person is itself influenced by societal factors. For example, women are more often than men expected to play the role of a home-maker, and so may not work as many hours.
