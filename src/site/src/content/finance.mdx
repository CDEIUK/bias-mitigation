---
title: Finance - the Adult dataset
---
...
The first dataset we will be considering is the Adult dataset from the <OutboundLink href="https://archive.ics.uci.edu/ml/datasets/Adult">UCI Machine learning repository</OutboundLink>. It was derived from the US Census, and contains demographic information from several thousand individuals. The typical task on this dataset is to predict whether or not an individual is a high earner from their demographics. You can see the details of our data acquisition and preprocessing in <OutboundLink href="https://mybinder.org/v2/gh/CDEIUK/bias-mitigation/master?filepath=notebooks%2Ffinance%2Fpreprocessing.ipynb">this notebook</OutboundLink>, and you can see the details of the below data exploration in <OutboundLink href="https://mybinder.org/v2/gh/CDEIUK/bias-mitigation/master?filepath=notebooks%2Ffinance%2Fdata.ipynb">this notebook</OutboundLink>.
...
<Collapse label="Features in the adult dataset">
...
Please note that in this dataset (drawn from the US Census), legacy categories for ethnic background exist that are now inappropriate. The graphs shown here are built automatically from that data. The data label `race` includes the category "Eskimo" - but where "American Indian / Eskimo" appears here, the correct terms would be Indigenous peoples, American Indian, Native American, or Alaska Native, depending on context.
