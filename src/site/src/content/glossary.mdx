...
We define a de-identification technique as any data transformation or modification that reduces the amount of information about an individual or entity in a dataset, and/or reduces the risk that an individual or entity can be re-identified. These methods are distinguished from the PETs described above in that they involve direct modification of data before it is shared or disseminated, rather than provided mechanisms for protecting confidentiality whilst maintaining maximum utility over the underlying data.

<Collapse label="Details">

Examples of de-identification techniques are:
...
- _**Generalization:**_ transforming a value to a less precise or bucketed value, e.g. replacing a height of 179cm to 170-180cm
